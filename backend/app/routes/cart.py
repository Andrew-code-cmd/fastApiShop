...
class AddToCartRequest(BaseModel): # этот класс чисто для формализации запроса. "Дай мне запрос
    product_id: int                # в таком формате"
    quantity: int
    cart: Dict[int, int] = {}


class UpdateCartRequest(BaseModel): # и этот тоже
    product_id: int
    quantity: int
    cart: Dict[int, int] = {}


class RemoveFromCartRequest(BaseModel): # и этот
    cart: Dict[int, int] = {}
